...
import { init, openHistory, type Project, tempDir } from "./helpers.ts";
...
function randomMutation(p: Project, rand: () => number, seq: { n: number }): void {
  ...
  if (r < 0.3 || track.clips.length === 0) {
    ...
    p.meta = {
      ...(p.meta ?? {}),
      [`k${Math.floor(rand() * 5)}`]: rand() < 0.5 ? Math.floor(rand() * 10) : { nested: [rand() < 0.5, "x"] },
    };
...
describe("性質: HEAD.after は常に現在の project のハッシュ", () => {
  test.each([1, 7, 42])(
    "seed %i: 200 変更 + ランダムな undo/redo/checkout/commit",
    async (seed) => {
      const rand = rng(seed);
      const seq = { n: 1 };
      ...
      while (edits < 200) {
        ...
        if (r < 0.55) {
          ...
          const res = await h.recordOp({
            before: current,
            after,
            command: ["rand", String(edits)],
            actor: rand() < 0.5 ? "ai" : "human",
            summary: `edit ${edits}`,
          });
