...
import { createContext, detectActor, findProjectDirFrom, type GlobalOptions } from "../../../src/cli/context.ts";
...
describe("output envelopes", () => {
  ...
  test("failure envelope shape", () => {
    ...
    expect(env).toEqual({
      ok: false,
      command: "clip add",
      error: { code: "E_X", message: "msg", hint: "h", detail: { a: 1 } },
    });
  });
  ...
  test("human mode prints human text and warnings to stderr", () => {
    ...
    printSuccess(
      ...
      {
        human: "all good",
        warnings: [{ code: "W_A", message: "careful", hint: "do x" }],
      },
    );
  ...
  test("failure in json goes to stdout; in human mode to stderr", () => {
    ...
    expect(JSON.parse(j.out[0]!)).toMatchObject({
      ok: false,
      error: { code: "E_Y" },
    });
    const h = captureCtx();
...
describe("context", () => {
  ...
  test("requireProjectDir throws E_PROJECT_NOT_FOUND", () => {
    ...
    try {
      ctx.requireProjectDir();
    } catch (e) {
      expect((e as MontashError).code).toBe("E_PROJECT_NOT_FOUND");
    }
  });
  test("-C / MONTASH_PROJECT resolve explicitly", () => {
    const root = mkdtempSync(join(tmpdir(), "montash-explicit-"));
    writeFileSync(join(root, "project.json"), "{}");
    const ctx = createContext(globals({ project: root }), {
      cwd: tmpdir(),
      env: {},
      isTTY: false,
    });
    expect(ctx.findProjectDir()).toBe(root);
    const ctx2 = createContext(globals(), {
      cwd: tmpdir(),
      env: { MONTASH_PROJECT: root },
      isTTY: false,
    });
