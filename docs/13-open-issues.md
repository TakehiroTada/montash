...
| D-13 | 中 | **project.json のマイグレーション機構が無い**。`schema_version` は単一のグローバル整数で、旧版を読むと `automatic migration is not implemented yet.` のヒント付きで `E_PROJECT_INVALID` になる（`core/project.ts:200`）。プラグインでデータ形式が動くと移行手段が無い | `core/migrate.ts` を新設し `loadProject` が段階的に変換する。変換は `actor: system` の op として記録し `pre-migrate-v<N>` タグを打つ（docs/05 §13 の設計を実装） | open（計画: P0-1） |
| D-14 | **高** | **クリップ種別の union が閉じている**。`TrackClipSchema`（`core/schema.ts:282`）は 4 種の union で、未知の種別が 1 つあると project.json **全体**が `E_PROJECT_INVALID` で開けなくなる。「未知キーは `looseObject` で保持する」方針が種別単位では成立していない。さらに種別判定が `core/schema.ts:398` / `server/computed.ts:83` / `web/lib/timeline.ts:79` に三重実装され、未知種別は黙って `media` 扱い（= 1 フレーム幅で描画）になる | `type` を全クリップ必須の判別キーにし（media は `type: "media"`）、`discriminatedUnion` + `OpaqueClipSchema`（`type`/`id`/`start_f`/`duration_f` だけ検証し他は保持）に。判定は `core` の 1 関数へ統合。レンダー時のみ `E_PLUGIN_MISSING`、`validate` は `W_UNKNOWN_CLIP_TYPE` 警告 | open（計画: P0-2） |
| D-15 | 中 | **エフェクトの受け皿はあるがレンダラが拒否する**。`EffectSchema`（`core/schema.ts:191`）は `{type, params, keyframes}` の汎用形で完全に開いているのに、`graph/builder.ts:72` と `graph/video.ts:93` が `unsupported("clip effects")` で即拒否。`effects` を持てるのも media クリップだけ | `src/registry/effects.ts` を新設して `defineEffect()` を導入。`effects` を全クリップ種別に持たせ、`normalizeVideoClip()` の `format=` 直前でレジストリを展開（音声は `normalizeAudioClip()`）。組み込みの色補正（`colorFilter()` の eq）を最初の builtin として載せ替える | open（計画: P0-3） |
| D-16 | 低 | **同じ概念のプリセットが二重管理**。オーバーレイの配置（`graph/overlay.ts:23 PRESETS`）と ASS のテキスト位置（`ffmpeg/ass.ts:87 POSITION_PRESETS`）が別実装。一方で `text_presets` / `render_presets` は「組み込み + project 由来のマージ + `source` ラベル」という良い形を既に持つ | 位置プリセットを `src/registry/positions.ts` に統合し、マージ方式を `createRegistry<T>()` として `src/registry/index.ts` に共通化。`text_presets` / `render_presets` もその上に載せる | open（計画: P0-4） |
| D-17 | 低 | **ID プレフィックスが 1 文字固定で復旧できない**。`nextId()` は任意文字列を受けるのに、`ID_PATTERN`（`core/ids.ts:25`）が英小文字 1 文字 + 数字で、`collectNumberedIds` / `rebuildIds` は既知 5 種しか走査しない。`ids.json` を失うとプラグイン側のカウンタだけ 1 に巻き戻り ID が重複する。`existingIds` もプラグイン領域を見ない | `ID_PATTERN` を多文字プレフィックス対応にし、再構築はレジストリに宣言されたプレフィックスを走査。`existingIds` が `project.plugins.*[].id` も対象にする | open（計画: P0-5） |
