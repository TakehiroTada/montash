...
- **プレフィックスは 1 文字固定ではない**。`^[a-z][a-z0-9]*?(\d+)$`（末尾の数字列を最短一致で切り出す）に合う多文字プレフィックスを使える（プラグイン用の `fx1`, `fx12` など）。末尾の数字列が連番なので、**プレフィックス自体を数字で終えることはできない**（`fx2` はプレフィックスにできない）。組み込み 1 文字 ID の解釈は従来どおり（`c1` = `c` + `1`）。
- **プレフィックスは登録制**。組み込みは `c` / `t` / `x` / `s` / `d` の 5 種で、それ以外は `registerIdPrefix()`（`core/ids.ts`）で登録したものだけが収集・再構築の対象になる。登録されていない文字列（アセット slug の `clip1` など）はカウンタを汚さない。`nextId()` で発行したプレフィックスは自動的に登録される。
- カウンタは **`.montash/ids.json`** に置き、`project.json`（= 履歴の対象）には**含めない**。`checkout` で過去に戻っても採番は巻き戻らないので、別系列で同じ ID が生まれない。

```json
{ "counters": { "c": 12, "t": 3, "x": 2, "s": 1, "d": 1, "fx": 4 } }
```

- `ids.json` が無い／壊れている場合、`montash ids rebuild`（または次回コマンド実行時に自動）が現在の `project.json` と `.montash/history/objects/*` を走査して各プレフィックスの最大値 +1 に復元する。走査対象は **登録済みプレフィックス + 既存 `ids.json` に載っているプレフィックス**。未登録のプレフィックスは `ids.json` を完全に失うと復元できないため、プラグインは起動時に自分のプレフィックスを登録する。
- 復元は既存カウンタを**下げない**（`ids.json` にしかないプレフィックスもそのまま維持する）。
- `--id` で明示指定した ID が現在の `project.json` に存在すれば `E_ID_EXISTS`、過去の object のいずれかに存在すれば `W_ID_REUSED`。重複検査は `project.json` の**任意のサブツリー**（`plugins.*[].id` などプラグイン領域を含む）の `id` フィールドを再帰的に集めて行う。
