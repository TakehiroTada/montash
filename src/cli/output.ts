...
import type { MontashError, Warning } from "./errors.ts";
...
const colorEnabled = (ctx: CommandContext): boolean =>
  !ctx.globals.noColor && Boolean(process.stdout.isTTY) && !("NO_COLOR" in ctx.env);
const paint = (ctx: CommandContext, code: string, s: string): string =>
  colorEnabled(ctx) ? `\x1b[${code}m${s}\x1b[0m` : s;
