...
import { ExitCode, MontashError, type Warning, warning } from "../errors.ts";
...
export const validate = defineCommand<Args>({
  ...
  options: {
    deep: {
      type: "boolean",
      describe: "also check that asset files exist (ffmpeg decode check: not implemented yet)",
      default: false,
    },
  ...
  async handler(ctx, args) {
    ...
    if (args.deep) {
      ...
      warnings.push(
        warning(
          "W_DEEP_NOT_IMPLEMENTED",
          "--deep checked file existence only; ffmpeg decode and font checks are not implemented yet",
        ),
      );
    }
    const result = {
