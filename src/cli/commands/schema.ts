...
export const schema = defineCommand<Args>({
  ...
  positionals: [
    {
      name: "command",
      describe: 'command path, e.g. "clip trim" (default: all)',
    },
  ],
  options: {
    format: {
      ...
      choices: ["json", "anthropic-tools", "openai-tools"],
      default: "json",
    },
  },
  examples: [
    ...
    {
      cmd: "montash schema --format anthropic-tools > tools.json",
      note: "LLM tool definitions",
    },
  ...
  async handler(_ctx, args) {
    ...
    return {
      ...
      human: () =>
        args.format === "json"
          ? specs.map((s) => `${s.path.padEnd(24)} ${s.summary}`).join("\n")
