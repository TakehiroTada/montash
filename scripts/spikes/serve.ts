...
await Bun.write(
	big,
	new Uint8Array(1_000_000).map((_, i) => i % 251),
);
const srv = Bun.serve({
	port: 0,
	hostname: "127.0.0.1",
	fetch(req, server) {
		...
		if (url.pathname === "/manual") {
			// manual Range via slice
			const f = Bun.file(big);
			const r = req.headers.get("range");
			if (!r) return new Response(f);
			const [, s, e] = /bytes=(\d*)-(\d*)/.exec(r)!;
			const start = s ? +s : 0;
			const end = e ? Math.min(+e, f.size - 1) : f.size - 1;
...
for (const p of ["/auto", "/manual"]) {
	...
	console.log(
		`${p}: status=${res.status} len=${len} content-range=${res.headers.get("content-range")} accept-ranges=${res.headers.get("accept-ranges")}`,
	);
}
