...
import { watch, writeFileSync, mkdirSync, renameSync, rmSync } from "node:fs";
...
// ---- A) chokidar@4 ----
{
  ...
  const events: string[] = [];
  const w = chokidar.watch(dir, { ignoreInitial: true, awaitWriteFinish: { stabilityThreshold: 50, pollInterval: 10 } });
  w.on("all", (ev, p) => events.push(`${ev}:${p.split("/").pop()}`));
  await new Promise<void>((r) => w.on("ready", () => r()));
  await Bun.sleep(100);
  writeFileSync(target, JSON.stringify({ a: 1 }));                                  // 上書き
  await Bun.sleep(200);
  writeFileSync(`${target}.tmp`, JSON.stringify({ b: 2 })); renameSync(`${target}.tmp`, target); // 原子的保存（vedit の保存方式）
  await Bun.sleep(200);
  mkdirSync(`${dir}/.vedit/history`, { recursive: true });
  writeFileSync(`${dir}/.vedit/history/ops.jsonl`, "line\n");                       // サブディレクトリへの追加
  await Bun.sleep(300);
  await w.close();
  const okA = events.some((e) => e.startsWith("change:project.json")) && events.some((e) => e.startsWith("add:ops.jsonl"));
...
// ---- B) Bun native fs.watch (recursive) — comparison ----
{
  ...
  const events: string[] = [];
  const w = watch(root, { recursive: true }, (ev, f) => events.push(`${ev}:${f}`));
  await Bun.sleep(100);
  writeFileSync(`${root}/project.json`, "{}");
  writeFileSync(`${root}/p.tmp`, "{}"); renameSync(`${root}/p.tmp`, `${root}/project.json`);
  writeFileSync(`${root}/.vedit/history/ops.jsonl`, "x\n");
  await Bun.sleep(300);
  w.close();
  const okB = events.some((e) => e.includes("project.json")) && events.some((e) => e.includes("ops.jsonl"));
  console.log("fs.watch recursive events:", JSON.stringify(events));
  console.log("RESULT fs.watch (informational, not required):", okB ? "OK" : "FAIL");
}
